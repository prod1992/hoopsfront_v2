...
class SelectDropdown extends React.Component {
  ...
  sortDataByName(event) {
    let sortedArray = [],
      ...
      list =
        this.props.selectVendor.vendorList &&
        this.props.selectVendor.vendorList.data;
    //let filteredIds;
    for (let i in list) {
  ...
  render() {
    ...
    return (
      <div className="select-drop">
        <div className="value" onClick={() => this.toggleDropdown()}>
          {selectVendor.selected === null
            ? "Select drop"
            : selectVendor.selected["vendor_name"]}
        </div>
        {this.state.dropOpen && (
          <div className="dropdown">
            ...
            <div className="options">
              <div className="shared-scroll-view">
                <button className="add-new" onClick={() => this.addNewVendor()}>
                  Add new
                </button>
                {selectVendor.vendorList.data &&
                  selectVendor.vendorList.data.map(item => (
                    <div
                      className="option"
                      key={item.id}
                      onClick={() => this.hendleOptionValue(item)}
                    >
                      {item["vendor_name"]}
                    </div>
                  ))}
